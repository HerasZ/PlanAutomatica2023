...
def main():
    ...
    for x in range(options.drones):
        ...
        #Dos brazos /dron
        brazos.append("brazod_dron"+str(x+1))
        brazos.append("brazoi_dron"+str(x+1))
    ...
    with open(problem_name + ".pddl", 'w') as f:
        ...
        #Inicializar drones en base
        i=0
        for x in drone:
            dron_brazos = []
            f.write("\t(loc-dron base " + x + ")\n")
            dron_brazos.append(brazos[i])
            i+=1
            dron_brazos.append(brazos[i])
            f.write("\t(brazos-dron "+ x +" "+ dron_brazos[0] +")\n")
            f.write("\t(libre-brazo "+ x +" "+ dron_brazos[0] +")\n")
            f.write("\t(brazos-dron "+ x +" "+ dron_brazos[1] +")\n")
            f.write("\t(libre-brazo "+ x +" "+ dron_brazos[1] +")\n")
            f.write("\t(distintos-brazos-dron "+ dron_brazos[0] +" "+ dron_brazos[1] +")\n")
            i+=1
        ...
        #Inicializar localizaciones personas
        for x in person:
            randomNum = random.randint(0, len(location)-1)  
            while(location[randomNum] == "base"):
                randomNum = random.randint(0, len(location)-1)
            f.write("\t(loc-persona " + location[randomNum] + " " + x + ")\n")
